...
/**
 * Get a log file path in the user data directory
 */
const char* get_log_file_path(const char* log_name) {
    static char log_path[USER_DATA_MAX_PATH];
    ...
    snprintf(log_path, sizeof(log_path), "%s%slogs%s%s", user_data_dir, PATH_SEPARATOR, PATH_SEPARATOR, log_name);
    return log_path;
}
...
const char* get_default_log_file_path(void) {
    static char default_log_path[USER_DATA_MAX_PATH];

    if (!initialized && !init_user_data_directory()) {
        return NULL;
    }

    snprintf(default_log_path, sizeof(default_log_path), "%s%slogs%sintercept.log", user_data_dir, PATH_SEPARATOR, PATH_SEPARATOR);
