...
#[tauri::command]
async fn setup_connection_callbacks(app_handle: tauri::AppHandle) -> Result<(), String> {
    set_app_handle(app_handle);

    let lib = get_library()?;
    let lib = lib.lock().map_err(|e| format!("Failed to acquire library lock: {}", e))?;

    // Set up the connection and disconnect callbacks
    lib.set_connection_callback(connection_callback)
        .map_err(|e| format!("Failed to set connection callback: {}", e))?;
...
#[tauri::command]
async fn get_proxy_status() -> Result<ProxyConfigResponse, String> {
    let lib = get_library()?;
    let lib = lib.lock().map_err(|e| format!("Failed to acquire library lock: {}", e))?;

    lib.get_proxy_config()
        .map_err(|e| format!("Failed to get proxy configuration: {}", e))
}

#[tauri::command]
async fn get_proxy_history() -> Result<Vec<ProxyHistoryEntry>, String> {
    let storage = get_proxy_history_storage();
    let history = storage.lock().map_err(|e| format!("Failed to acquire proxy history lock: {}", e))?;
    Ok(history.clone())
}

#[tauri::command]
async fn clear_proxy_history() -> Result<(), String> {
    let storage = get_proxy_history_storage();
    let mut history = storage.lock().map_err(|e| format!("Failed to acquire proxy history lock: {}", e))?;
    history.clear();
    Ok(())
}

#[tauri::command]
async fn clear_selected_proxy_history(entry_ids: Vec<i32>) -> Result<(), String> {
    let storage = get_proxy_history_storage();
    let mut history = storage.lock().map_err(|e| format!("Failed to acquire proxy history lock: {}", e))?;

    // Remove entries that match the provided entry IDs (using packet_id as unique identifier)
    history.retain(|entry| !entry_ids.contains(&entry.packet_id));

    Ok(())
}

#[tauri::command]
async fn get_intercept_status() -> Result<InterceptStatusResponse, String> {
    let lib = get_library()?;
    let lib = lib.lock().map_err(|e| format!("Failed to acquire library lock: {}", e))?;

    lib.get_intercept_config()
        .map_err(|e| format!("Failed to get intercept configuration: {}", e))
}

#[tauri::command]
async fn set_intercept_enabled(enabled: bool) -> Result<(), String> {
    let lib = get_library()?;
    let lib = lib.lock().map_err(|e| format!("Failed to acquire library lock: {}", e))?;

    lib.set_intercept_enabled(enabled)
        .map_err(|e| format!("Failed to set intercept enabled: {}", e))?;

    // Notify the frontend that intercept has been enabled/disabled
    if let Some(app_handle) = get_app_handle() {
        let status_msg = format!("Interception {}", if enabled { "enabled" } else { "disabled" });
        if let Err(e) = app_handle.emit("status-message", &status_msg) {
            eprintln!("Failed to emit intercept status message: {}", e);
        }
    }
...
#[tauri::command]
async fn set_intercept_direction(direction: i32) -> Result<(), String> {
    let lib = get_library()?;
    let lib = lib.lock().map_err(|e| format!("Failed to acquire library lock: {}", e))?;

    lib.set_intercept_direction(direction)
        .map_err(|e| format!("Failed to set intercept direction: {}", e))?;

    // Notify the frontend that intercept direction has been changed
    if let Some(app_handle) = get_app_handle() {
        ...
        let status_msg = format!("Intercept direction set to: {}", direction_str);
        if let Err(e) = app_handle.emit("status-message", &status_msg) {
            eprintln!("Failed to emit intercept direction message: {}", e);
        }
    }
...
#[tauri::command]
async fn respond_to_intercept(connection_id: i32, packet_id: i32, action: i32, data: Vec<u8>) -> Result<(), String> {
    println!("RUST TAURI: respond_to_intercept called with:");
    println!("  connection_id: {}", connection_id);
    ...
    // Handle the intercept response first
    {
        let lib = get_library()?;
        let lib = lib.lock().map_err(|e| format!("Failed to acquire library lock: {}", e))?;

        println!("RUST TAURI: Calling C library respond_to_intercept with connection_id: {}, action: {}, data: {:?}", connection_id, action, data);

        // Call C function to respond to the intercepted packet
        lib.respond_to_intercept(connection_id, action, &data)
            .map_err(|e| format!("Failed to respond to intercept: {}", e))?;

        println!("RUST TAURI: C library call completed successfully");
    } // Release the lock here

    // If packet was modified (action = 2), update proxy history with edited data
    if action == 2 {
        let edited_data_str = String::from_utf8_lossy(&data).to_string();
        if let Err(e) = update_proxy_history_entry(packet_id, edited_data_str).await {
            eprintln!("Failed to update proxy history with edited data: {}", e);
        } else {
            println!("Updated proxy history entry for packet_id: {} with edited data", packet_id);
        }
    }

    // Clean up the intercepted data from memory
    if let Ok(mut storage) = get_intercepted_data_storage().lock() {
        // Find the index of the item with matching connection_id and packet_id
        if let Some(index) = storage.iter().position(|item|
            item.connection_id == connection_id && item.packet_id == packet_id
        ) {
            // Remove the item from storage
            storage.remove(index);
            println!("RUST TAURI: Cleaned up intercepted data for connection_id: {}, packet_id: {}", connection_id, packet_id);
        } else {
            println!("RUST TAURI: No intercepted data found for cleanup - connection_id: {}, packet_id: {}", connection_id, packet_id);
        }
    } else {
        eprintln!("Failed to acquire lock on intercepted data storage for cleanup");
    }
    ...
    let action_str = match action {
        0 => "forwarded",
        1 => "dropped",
        2 => "modified and forwarded",
        _ => "processed with unknown action",
    };

    println!("Packet #{} from connection #{} has been {} and removed from memory",
        packet_id, connection_id, action_str);

    // Notify the frontend that intercept has been responded to
    if let Some(app_handle) = get_app_handle() {
        let status_msg = format!("Intercepted packet #{} {}", connection_id, action_str);
        if let Err(e) = app_handle.emit("status-message", &status_msg) {
            eprintln!("Failed to emit intercept response message: {}", e);
        }
    }
...
#[tauri::command]
async fn setup_intercept_callback() -> Result<(), String> {
    let lib = get_library()?;
    let lib = lib.lock().map_err(|e| format!("Failed to acquire library lock: {}", e))?;
    ...
    // Initialize/clear the intercept data storage
    if let Ok(mut storage) = get_intercepted_data_storage().lock() {
        storage.clear(); // Clear any existing data to start fresh
    } else {
        eprintln!("Failed to initialize intercept data storage");
    }

    println!("Intercept callback registered successfully and storage initialized");
    Ok(())
}

#[tauri::command]
async fn get_intercepted_data() -> Result<Vec<InterceptedData>, String> {
    let storage = get_intercepted_data_storage();
    let data = storage.lock().map_err(|e| format!("Failed to acquire intercepted data lock: {}", e))?;
    Ok(data.clone())
}
...
async fn update_proxy_history_entry(packet_id: i32, edited_data: String) -> Result<(), String> {
    ...
    if let Some(entry) = history.iter_mut().find(|entry| entry.packet_id == packet_id) {
        entry.modified = true;
        entry.edited_data = Some(edited_data);
        println!("Updated proxy history entry for packet_id: {}", packet_id);
        Ok(())
    } else {
        Err(format!("No proxy history entry found with packet_id: {}", packet_id))
    }
}
